...
This example app lets the user chat with Gemini and use it as a personal AI
assistant. The app supports text-only chat in two modes: non-streaming and
streaming.

In non-streaming mode, a response is returned after Gemini completes the entire
text generation process.

Streaming mode uses Gemini's streaming capability to achieve faster
interactions.
...
The example provides three implementations of the backend server:
